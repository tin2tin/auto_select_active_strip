...
bl_info = {
    ...
    "version": (1, 4),
    "blender": (2, 90, 0),
    "location": "Sequencer > Header > Auto Select",
...
class autoselect_property_group(PropertyGroup):
    ...
    select_mode: EnumProperty(
        ...
        items=(
            ...
            (
                "CHANNEL",
                "Channel",
                "Select only strip in chosen channel under playhead",
                2,
            ),
        ),
    )
    ...
    channel: IntProperty(
        name="Channel", description="Restrict selection to this channel", default=1
    )

    select_while_playing: BoolProperty(
        name="While Playing",
        description="Selection is still active during playback",
        default=True,
    )

    select_handles: BoolProperty(
        name="Handles", description="Selectable handles", default=True
    )
...
class SEQUENCER_PT_auto_select_ui(Panel):
    ...
    def draw_header(self, context):
        ...
        ico = (
            "RESTRICT_SELECT_OFF"
            if settings.auto_select_toggle
